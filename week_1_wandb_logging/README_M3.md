# M3芯片Mac运行指南 - Week 1 (W&B Logging)
...
```bash
# 使用conda创建虚拟环境
conda create --name wandb-m3 python=3.9
conda activate wandb-m3
```
...
**注意**：如果遇到tokenizers并行警告，可以设置环境变量：
```bash
...
python -m ipykernel install --user --name wandb-m3
...
3. **批处理大小**: 如果遇到内存不足，可以在 `data.py` 中减小 `batch_size` 参数
